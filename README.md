...
|---|---|---|---|---|
| `ec2-default` | Bare EC2 + SSM (no software) | none (SSM only) | — | `amazon-linux`, `ubuntu`, `rhel9` |
...
3. Set `stack_name`, `instance_type` and `os` (`amazon-linux` = Amazon Linux 2023, `ubuntu` = Ubuntu 24.04 LTS, `rhel9` = RHEL 9.8.0; all x86_64). Where a branch supports it, `ami_id` optionally overrides the per-OS default. Default AMI IDs are pinned in the workflow's `Resolve AMI` step — edit there to bump versions.
